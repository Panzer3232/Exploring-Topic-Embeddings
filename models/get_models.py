...
def get_model(model_name, vocab_size, vocab_embeddings, topic_size):
    if model_name == 'dvae':
        ...
        model = ETM(vocab_size=vocab_size,
                    vocab_embeddings=vocab_embeddings, topic_size=topic_size)
    elif model_name == 'etm1':
        # writer = SummaryWriter('runs/my_experiment')
        model = ETM(vocab_size=vocab_size,
                    vocab_embeddings=vocab_embeddings, topic_size=topic_size)
    elif model_name == 'etm_dirichlet':
        model = ETMD(vocab_size=vocab_size,
                     vocab_embeddings=vocab_embeddings, topic_size=topic_size)

    elif model_name == 'etmd_doc':
        model = ETMD(vocab_size=vocab_size,
                     vocab_embeddings=vocab_embeddings, topic_size=topic_size)

    elif model_name == 'etm_dirichlet_rsvi':
        model = ETMD_RSVI(
            vocab_size=vocab_size, vocab_embeddings=vocab_embeddings, topic_size=topic_size)
